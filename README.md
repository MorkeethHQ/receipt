...
- **Fine-Tuning**: Quality-gated training pipeline. After `createFineTuningTask()`, attempts `resolveAdapterName()` then `deployAdapterByName()` to close the loop.
...
- **[Live Demo](https://receipt-murex.vercel.app/demo)**: Watch Researcher + Builder generate receipts in real-time. Honest mode + adversarial tamper detection.
...
- **[Team](https://receipt-murex.vercel.app/team)**: Multi-agent chain feed. Aggregates receipt chains from Claude Code hooks and OpenClaw plugin. Filter by source, verify any chain.
...
- **[Eval](https://receipt-murex.vercel.app/eval)**: Constitutional AI evaluation harness. 60 test cases, 3 model evaluators, self-critique loop, consensus accuracy.
- **[Reputation](https://receipt-murex.vercel.app/reputation)**: Agent leaderboard with verification rate and cost-per-useful-output analysis.
